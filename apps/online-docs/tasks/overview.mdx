...
<Info>
**Renamed from Hooks:** Tasks were previously called "Hooks". If you have an existing `hooks.json` file, it will be automatically migrated to `tasks.json` on workspace load. A backup is kept at `hooks.json.old`. Both formats are supported during the transition.
</Info>
...
Save this as `~/.craft-agent/workspaces/{workspaceId}/tasks.json` and it starts working immediately — no restart needed.
...
Tasks are configured per-workspace in `tasks.json`:
