...
<Warning>
**Label values are not yet fully implemented on the UI, coming soon.** The configuration and storage layer supports values, but the UI does not yet expose full value editing or display.
</Warning>
...
| `color` | EntityColor | No | Color for the label circle (see below) |
| `valueType` | `"string"` \| `"number"` \| `"date"` | No | Value type hint. Omit for presence-only labels. |
